# ============================================================================
# MODULE 07B: COMPREHENSIVE PEATLAND MRV STANDARDS COMPLIANCE REPORT
# ============================================================================
...
#          actionable recommendations for peatland carbon projects
#
# STANDARDS COVERED:
#   - VM0036 (Verra) - Methodology for Rewetting Drained Temperate Peatlands
#   - CaMP - Canadian Model for Peatlands (NRCan national framework)
#   - IPCC 2013 Wetlands Supplement (Chapter 2: Peatlands)
#   - ECCC - Environment and Climate Change Canada Wetland Standards
#
# CANADIAN PEATLAND-SPECIFIC CRITERIA:
#   - Water table monitoring (VM0036 critical parameter)
#   - Peat depth to mineral soil
#   - GHG emissions reduction verification
#   - Rewetting effectiveness assessment
#
# INPUTS:
...
#   - Field data quality metrics (pH, von Post, water table)
#
# OUTPUTS:
...
log_message("=== MODULE 07B: COMPREHENSIVE PEATLAND MRV STANDARDS COMPLIANCE REPORT ===")
...
# VM0036 (Verra) Requirements - Peatland Rewetting
vm0036_criteria <- list(
  name = "VM0036 - Methodology for Rewetting Drained Temperate Peatlands",
  requirements = list(
    list(
      criterion = "Minimum samples per stratum",
      threshold = VM0036_MIN_CORES,
      check_function = function(data) {
        ...
        pass <- all(samples_per_stratum$n_cores >= VM0036_MIN_CORES)

        message <- if (pass) {
          sprintf("All strata have ≥%d samples (min: %d)", VM0036_MIN_CORES, min_samples)
        } else {
          failing_strata <- samples_per_stratum %>%
            filter(n_cores < VM0036_MIN_CORES)
    ...
    list(
      ...
      threshold = paste0("≤", VM0036_TARGET_PRECISION, "%"),
      check_function = function(data) {
        ...
        pass <- max_error <= VM0036_TARGET_PRECISION

        message <- if (pass) {
          sprintf("All strata within target (max: %.1f%%)", max_error)
        } else {
          failing_strata <- stocks %>% filter(relative_error > VM0036_TARGET_PRECISION)
          sprintf("FAIL: %d strata exceed %.0f%% (max: %.1f%%)",
                  nrow(failing_strata), VM0036_TARGET_PRECISION, max_error)
        }
    ...
    list(
      ...
      check_function = function(data) {
        ...
        # Check if all VM0036 depths are represented
        required_depths <- VM0036_DEPTH_MIDPOINTS
        ...
        message <- if (has_all_depths) {
          "All VM0036 standard depths present"
        } else {
      ...
      check_function = function(data) {
        ...
        if (has_conservative) {
          ...
          message <- if (all_conservative_lower) {
            ...
            "FAIL: Some conservative estimates ≥ mean"
          }
    ...
    list(
      ...
      threshold = paste0("Every ", VM0036_MONITORING_FREQUENCY, " years"),
      check_function = function(data) {
...
for (method in c("rf", "kriging")) {
  vm0036_file <- sprintf("outputs/carbon_stocks/carbon_stocks_conservative_vm0036_%s.csv", method)
  if (file.exists(vm0036_file)) {
    stocks <- tryCatch({
      read.csv(vm0036_file)
    }, error = function(e) {
...
if (!is.null(workflow_data$cores)) {
  ...
  if (nrow(understrata) > 0) {
    for (i in 1:nrow(understrata)) {
      needed <- VM0036_MIN_CORES - understrata$n_cores[i]
      recommendations[[length(recommendations) + 1]] <- list(
        priority = "HIGH",
        category = "Sampling",
        recommendation = sprintf("Collect %d additional cores in '%s' stratum to meet VM0036 minimum (%d cores currently, %d required)",
                                needed, understrata$stratum[i], understrata$n_cores[i], VM0036_MIN_CORES),
        standard = "VM0036",
...
if (!is.null(workflow_data$carbon_stocks)) {
  high_uncertainty <- workflow_data$carbon_stocks %>%
    ...
    filter(relative_error > VM0036_TARGET_PRECISION)
  ...
  if (nrow(high_uncertainty) > 0) {
    for (i in 1:nrow(high_uncertainty)) {
      ...
      # Assuming CV ≈ 30%, z = 1.96, target = 20%
      required_samples <- ceiling((1.96 * VM0036_ASSUMED_CV / VM0036_TARGET_PRECISION)^2)
      ...
      if (additional_needed > 0) {
        recommendations[[length(recommendations) + 1]] <- list(
          ...
          recommendation = sprintf("Collect ~%d additional cores in '%s' to reduce uncertainty from %.1f%% to target %.0f%%",
                                  ...
                                  high_uncertainty$relative_error[i], VM0036_TARGET_PRECISION),
          standard = "VM0036",
...
if (MONITORING_YEAR == format(Sys.Date(), "%Y")) {
  recommendations[[length(recommendations) + 1]] <- list(
    priority = "MEDIUM",
    category = "Monitoring",
    recommendation = sprintf("Establish baseline monitoring. Next verification required in %d years (Year %d) per VM0036",
                            VM0036_MONITORING_FREQUENCY,
                            as.numeric(MONITORING_YEAR) + VM0036_MONITORING_FREQUENCY),
    standard = "VM0036",
    action = sprintf("Schedule %d monitoring", as.numeric(MONITORING_YEAR) + VM0036_MONITORING_FREQUENCY)
  )
}
...
if (!is.null(workflow_data$cores)) {
  html_content <- paste0(html_content, sprintf('
<li>✓ Harmonized core data (%d cores)</li>
', n_distinct(workflow_data$cores$core_id)))
} else {
  html_content <- paste0(html_content, '
<li>✗ Harmonized core data not found</li>
')
}
...
if (!is.null(workflow_data$carbon_stocks)) {
  ...
  html_content <- paste0(html_content, sprintf('
<li>✓ Carbon stock estimates (%s)</li>
', paste(methods, collapse = ", ")))
} else {
  html_content <- paste0(html_content, '
<li>✗ Carbon stock estimates not found</li>
')
}

if (!is.null(workflow_data$cv_results)) {
  html_content <- paste0(html_content, sprintf('
<li>✓ Cross-validation results (%d depth layers)</li>
', n_distinct(workflow_data$cv_results$depth_cm)))
} else {
  html_content <- paste0(html_content, '
<li>✗ Cross-validation results not found</li>
')
}
...
for (method in names(workflow_data$spatial_predictions)) {
  ...
  if (n_rasters > 0) {
    html_content <- paste0(html_content, sprintf('
<li>✓ %s spatial predictions (%d rasters)</li>
', method, n_rasters))
  }
}
...
<li><strong>Prepare verification documentation</strong> using outputs from Module 07 (VM0036 package)</li>
<li><strong>Submit to third-party verifier</strong> with all supporting data and QA/QC logs</li>
<li><strong>Establish monitoring schedule</strong> per VM0036 requirements (every 5 years)</li>
</ol>
</div>
...
for (i in 1:nrow(compliance_summary)) {
  row <- compliance_summary[i, ]
  symbol <- if (row$Compliance_Rate >= 90) "✓" else if (row$Compliance_Rate >= 70) "⚠" else "✗"
...
cat("\nOutput Files:\n")
cat("-------------\n")
cat("  📄 Comprehensive Report: outputs/reports/comprehensive_standards_report.html\n")
cat("  📊 Compliance Summary: outputs/reports/standards_compliance_summary.csv\n")
cat("  📋 Action Plan: outputs/reports/recommendations_action_plan.csv\n")

cat("\nKey Actions:\n")
cat("------------\n")
if (high_priority > 0) {
  cat(sprintf("  ⚠️  Address %d HIGH priority recommendations immediately\n", high_priority))
  ...
  cat("  ✓ No critical issues identified\n")
}
